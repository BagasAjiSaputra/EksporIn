...
export async function UpdateCommodityAction(formData: FormData) {
  try {
    ...
    if (!res.ok) {
      return { success: false, message: data.message || "Failed to update commodity" };
    }

    revalidatePath("/admin/commodity");
    return { success: true, message: data.message || "Komoditas berhasil diperbarui" };
  } catch (error) {
